...
Analyze each customer's recent emails and identify clients with unresolved issues that need a support ticket. Only suggest tickets for clients who appear to have a real technical problem, billing issue, or critical complaint.

Return JSON array. Each item:
- customer_id: string
- title: string (PT, max 10 words, infinitive verb, e.g. "Corrigir erro de sincronização de dados")
- description: string (PT, 2-3 sentences describing the problem and its impact)
- priority: "urgent"|"high"|"medium"|"low"
- tags: string[] (2-3 tags in PT lowercase, e.g. ["bug", "integração", "faturação"])

Priority: urgent=service down or data loss; high=major feature broken or financial impact; medium=usability issue; low=minor inconvenience.
Skip informational or promotional emails. Empty array if no tickets needed. JSON array only. No markdown.`
...
type EnrichedSuggestion = TicketSuggestion & {
...
export async function POST() {
  ...
  // Fetch recent inbound emails (last 45 days)
  ...
  // Exclude customers with open/in-progress ticket created in last 14 days
  ...
  const hasRecentTicket = new Set((recentTickets ?? []).map((t) => t.customer_id))

  const candidates = [...byCustomer.values()]
    .filter((e) => !hasRecentTicket.has(e.customer_id))
    .slice(0, 15)

  if (candidates.length === 0) {
    return NextResponse.json({ ok: true, suggestions: [], message: 'Todos os clientes já têm tickets recentes.' })
  }
  ...
  let claudeResults: TicketSuggestion[] = []
  try {
